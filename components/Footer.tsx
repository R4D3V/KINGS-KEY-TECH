  ...
  return (
    <footer className="bg-white border-t border-gray-200">
      <div className="px-6 md:px-16 lg:px-24 pt-16 pb-12 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <Link href="/" className="no-underline inline-block mb-6">
            <div className="flex items-center gap-3">
              <svg width="36" height="36" viewBox="0 0 60 60" fill="none">
                <path d="M30 2 L54 16 L54 44 L30 58 L6 44 L6 16 Z" fill="#111" stroke="#D4A843" strokeWidth="1.5"/>
                <path d="M16 38 L16 30 L21 35 L30 24 L39 35 L44 30 L44 38 Z" fill="none" stroke="#D4A843" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round"/>
              ...
              <span className="font-[family-name:var(--font-bebas)] text-xl tracking-widest text-gray-900">
                KINGS KEY <span style={{ color: "var(--color-brand)" }}>TECH</span>
              </span>
            </div>
          </Link>
          <p className="text-sm text-gray-500 leading-relaxed mb-6">
            Professional tech services delivered with precision, speed, and integrity. Your trusted technology partner.
          </p>
        ...
        <div>
          <div className="flex items-center gap-3 mb-7">
            ...
            <h4 className="text-[11px] font-bold tracking-[0.22em] uppercase text-[var(--color-brand)]">Services</h4>
          </div>
          <ul className="list-none p-0 m-0 space-y-3">
            {serviceLinks.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="text-sm text-gray-500 hover:text-gray-900 hover:pl-2 transition-all duration-300 no-underline flex items-center gap-2 group">
        ...
        <div>
          <div className="flex items-center gap-3 mb-7">
            <span className="block w-5 h-px bg-[var(--color-brand)]"/>
            <h4 className="text-[11px] font-bold tracking-[0.22em] uppercase text-[var(--color-brand)]">Navigation</h4>
          </div>
          <ul className="list-none p-0 m-0 space-y-3">
            {navLinks.map((l) => (
              <li key={l.href}>
        ...
        <div>
          <div className="flex items-center gap-3 mb-7">
            <span className="block w-5 h-px bg-[var(--color-brand)]"/>
            <h4 className="text-[11px] font-bold tracking-[0.22em] uppercase text-[var(--color-brand)]">Contact Us</h4>
          </div>
          <div className="space-y-5">
            {[
              { label: "Phone", value: "+971 558 251 439", href: "tel:+971558251439" },
              { label: "Email", value: "fahadkayondo19@gmail.com", href: "mailto:fahadkayondo19@gmail.com" },
              { label: "WhatsApp", value: "Chat with Fahad K", href: "https://wa.me/971558251439" },
            ].map((c) => (
              <div key={c.label}>
                <p className="text-[10px] font-bold tracking-[0.2em] uppercase text-gray-400 mb-1">{c.label}</p>
      ...
      <div className="px-6 md:px-16 lg:px-24 py-6 flex flex-col sm:flex-row justify-between items-center gap-3">
        <p className="text-xs text-gray-400">© {new Date().getFullYear()} Kings Key Tech. All rights reserved.</p>
        <p className="text-xs text-gray-400">Built by <span style={{ color: "var(--color-brand)" }}>Fahad K</span></p>
      </div>
    </footer>
  );
}
